...
export default function ShortAnswerExercise({ exercise, onAnswered }) {
  ...
  return <form className="exercise-card card" onSubmit={submit}><label className="short-answer"><strong>{exercise.statement}</strong><textarea rows="3" value={answer} disabled={loading || Boolean(result)} onChange={(event) => setAnswer(event.target.value)} placeholder="Digite sua resposta" /></label>{error && <p className="form-error">{error}</p>}<ExerciseFeedback result={result} />{!result && <button type="submit" disabled={!answer.trim() || loading}>{loading ? "Enviando..." : "Responder"}</button>}</form>;
}
