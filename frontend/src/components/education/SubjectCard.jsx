...
export default function SubjectCard({ link, grade, progress }) {
  ...
  return <article className="education-card card subject-card"><div className="subject-icon" aria-hidden="true">{subject.icon || subject.name.slice(0, 1)}</div><div className="subject-card-copy"><h2>{subject.name}</h2>{subject.description && <p>{subject.description}</p>}<ProgressBar value={progress} /></div><Link className="button" to={`/learn/subject/${link.id}`} state={{ gradeSubject: link, grade }}>Estudar</Link></article>;
}
